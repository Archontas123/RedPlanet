export class Vector2 {
    ...
    magnitudeSq() { return this.x * this.x + this.y * this.y; } // Helper for distanceSq
    distance(v) { return this.subtract(v).magnitude(); }
    distanceSq(v) { return this.subtract(v).magnitudeSq(); } // Calculate squared distance
    angle() { return Math.atan2(this.y, this.x); }
    static fromAngle(angle, magnitude = 1) {
         return new Vector2(Math.cos(angle) * magnitude, Math.sin(angle) * magnitude);
    }
    clone() { // Add clone method
        return new Vector2(this.x, this.y);
    }
    dot(v) { // Add dot product method
...
export function isLineOfSightClear(startPos, endPos, obstacles) {
     for (const obstacle of obstacles) {
          // Use the existing lineIntersectsRect function from this file
